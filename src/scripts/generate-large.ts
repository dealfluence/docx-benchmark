...
async function main() {
  ...
  const sections = [
    ...
    { title: "4. Warranties", text: "The Seller warrants that the goods are free of defects." },
    {
      ...
      text: "The Seller's maximum liability under this agreement shall be capped at $100,000.",
    },
    {
      title: "6. Indemnity",
    ...
    {
      title: "8. Notices",
      text: "All notices shall be sent to the parties' respective addresses as set forth in §5.",
    },
  ];
  ...
  paragraphXmls.push(`<w:p><w:r><w:t>Typing some. Typing some text</w:t></w:r></w:p>`);
  ...
  for (const sec of sections) {
    ...
    if (sec.withTrackChanges) {
      ...
      paragraphXmls.push(`<w:p><w:r><w:t>${sec.text}</w:t></w:r></w:p>`);
    }
  }
  ...
  doc.part._element = parser.parseFromString(docXml, "text/xml")
